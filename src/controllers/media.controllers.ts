...
class MediaController {
  ...
  public serveImageController(req: any, res: Response) {
  ...
  public serveVideoStreamController(req: any, res: any) {
    const range = req.headers.range
    if (!range) {
      return res.status(HttpStatusCode.BAD_REQUEST).send('Requires Range header')
    }
    const { name } = req.params
