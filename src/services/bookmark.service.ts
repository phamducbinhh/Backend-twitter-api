...
class BookmarkService {
  ...
  async unBookmark({ id, body }: { id: string; body: BookmarkTweetReqBody }) {
    // Create a new follow
    const { tweet_id } = body
    ...
    // Nếu tweet tồn tại, tạo bookmark
    if (!existingBookmark) {
      return handleResponse(HttpStatusCode.BAD_REQUEST, false, BOOKMARK_MESSAGES.NOT_BOOKMARKED)
    }

    // Nếu đã bookmark, xóa bản ghi khỏi bảng Follower
    await db.Bookmark.destroy({
      where: { user_id: id, tweet_id }
    })
    return handleResponse(HttpStatusCode.SUCCESS, true, BOOKMARK_MESSAGES.UNBOOKMARK_SUCCESSFULLY)
  }
}
